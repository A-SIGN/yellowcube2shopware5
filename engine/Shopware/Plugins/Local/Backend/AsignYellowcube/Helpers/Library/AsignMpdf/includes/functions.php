...
function ycurldecode_pathonly($url) {	// mPDF 5.5.03
	if (preg_match('/[?]/',$url)) {
...
function _ycstrspn($str1, $str2, $start=null, $length=null) {
...
function _ycstrcspn($str1, $str2, $start=null, $length=null) {
	$numargs = func_num_args();
	if ($numargs == 2) {
...
function _ycfgets (&$h, $force=false) {
	$startpos = ftell($h);
	$s = fgets($h, 1024);
	if ($force && preg_match("/^([^\r\n]*[\r\n]{1,2})(.)/",trim($s), $ns)) {
		$s = $ns[1];
...
function ycPreparePreText($text,$ff='//FF//') {
